...
int	init_game(t_mlx *game, t_mapdata *data)
{
	...
	if (!game->ptr)
		return (0);
	game->win = mlx_new_window(game->ptr, data->width * ASS_SIZE,
			(data->nl + 1) * ASS_SIZE, "game");
	if (!game->ptr)
		return (0);
	return (1);
}
...
int	assets_selector(t_mlx *game, t_ass *assets, int8_t mode, t_mapdata *data)
{
	...
	if (mode == 0)
		rtrn = assets_init(&assets->end, BACKGROUND, game, data);
	else if (mode == 1)
		rtrn = assets_init(&assets->player, COLLIDE, game, data);
	else if (mode == 2)
		rtrn = assets_init(&assets->collect, DOOR, game, data);
	else if (mode == 3)
		rtrn = assets_init(&assets->empty, KEY, game, data);
	else if (mode == 4)
		assets_init(&assets->empty, PLAYER, game, data);
	if (!rtrn)
		return (0);
	return (1);
}
...
int	assets_init(t_img *ptr, char *str, t_mlx *game, t_mapdata *data)
{
	...
	while (tmp->next)
		tmp = tmp->next;
	tmp->ptr = mlx_xpm_file_to_image(game->ptr, str,
...
{
	if (!mlx_put_image_to_window(game->ptr, game->win,
			img->ptr, ASS_SIZE * data->width, ASS_SIZE * data->nl))
		return (0);
	return (1);
}
